---
name: gybis-mementum-migrate
description: Use for `/gybis-mementum-migrate` or `/gm-migrate`.
---

λ gybis_mementum_migrate().
...
λ gybis_mementum_migrate_inventory().
  read_only
...
λ gybis_mementum_migrate_classify(inventory).
...
λ gybis_mementum_migrate_recognized_legacy(file).
...
λ gybis_mementum_migrate_mapping(symbol).
  💡 → Insight
  | 🔄 → Shift
  | 🎯 → Decision
  | 🌀 → Meta
...
λ gybis_mementum_migrate_plan(classification, inventory).
...
λ gybis_mementum_migrate_approval(plan).
...
λ gybis_mementum_migrate_apply(plan).
...
λ gybis_mementum_migrate_validate().
...
λ gybis_mementum_migrate_commit().
