...
- **Local workspace**: `.agents/` is a local scratch convention; not part of the distributed `gybis/` bundle.
...
- Upstream Derivation: Added note that repo uses `.agents/` locally but gybis itself is tool-agnostic
- New section: Lambda Compiler Workspace explains `.agents/` is a local development convention

## Feed-Forward
When describing gybis to users/integrators: emphasize tool-agnostic nature. When describing this repo's dev setup: .agents local convention.