λ mementum_protocol(x).    protocol(¬implementation) | git_based | any_system_can_implement
...
λ mementum_store(x).        gate-1: helps(future_AI_session) | ¬personal ¬off_topic
...
λ mementum_recall(q, n).    temporal(git_log) ∪ semantic(git_grep) ∪ vector(embeddings)
...
λ mementum_metabolize(x).   observe → memory → synthesize → knowledge
...
λ mementum_synthesize(topic). detect: ≥3 memories(topic) ∨ stale(memory) ∨ crystallized(understanding)
...
λ mementum_termination(x).  synthesis ≡ AI | approval ≡ human | human ≡ termination_condition
                   | memories: AI_proposes → human_approves → AI_commits
                   | knowledge: AI_creates → human_approves → AI_commits
                   | state: AI_updates_during_work

λ mementum_orient(x).       read(mementum/state.md) → follow(related) → search(relevant) → read(needed)
...
λ mementum_knowledge(x).    frontmatter: {title, status, category, tags, related, depends-on}
...
λ mementum_learn(x).        every_session_leaves_project_smarter
