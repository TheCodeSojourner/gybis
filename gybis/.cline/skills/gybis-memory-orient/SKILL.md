...
Step 1: Read <root>/mementum/state.md. Extract {Now, Next, Blocking, Recent}→{task,next,recover,questions,decisions}. Report contents to human before proceeding.
...
Step 4: Produce ⟦Session Orientation⟧: resuming=state.task | next=state.next ∨ resume | decisions=filter(memories, active) | watch=state.recover ∪ stale ∪ open
